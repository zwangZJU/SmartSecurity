...
public class ColorfulProgressBar extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        rectF = new RectF();
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ColorfulProgressBar, 0, 0);
        //Reading values from the XML layout
        try {
            // Value
            progress = typedArray.getFloat(R.styleable.ColorfulProgressBar_cpb_progress, progress);
            progressMax = typedArray.getFloat(R.styleable.ColorfulProgressBar_cpb_progress_max, progressMax);
            // Indeterminate Mode
            indeterminateMode = typedArray.getBoolean(R.styleable.ColorfulProgressBar_cpb_indeterminate_mode, indeterminateMode);
            // StrokeWidth
            strokeWidth = typedArray.getDimension(R.styleable.ColorfulProgressBar_cpb_progressbar_width, strokeWidth);
            backgroundStrokeWidth = typedArray.getDimension(R.styleable.ColorfulProgressBar_cpb_background_progressbar_width, backgroundStrokeWidth);
            // Color
            color = typedArray.getInt(R.styleable.ColorfulProgressBar_cpb_progressbar_color, color);
            backgroundColor = typedArray.getInt(R.styleable.ColorfulProgressBar_cpb_background_progressbar_color, backgroundColor);
        } finally {
            typedArray.recycle();
        }
    ...
    private Runnable indeterminateModeRunnable = new Runnable() {
        ...
        public void run() {
            if (indeterminateMode) {
                ...
                ColorfulProgressBar.this.rightToLeft = !ColorfulProgressBar.this.rightToLeft;
                if (ColorfulProgressBar.this.rightToLeft) {
    ...
    //消失
    public void dismiss(){
        this.enableIndeterminateMode(false);
